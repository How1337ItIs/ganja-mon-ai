...
// Preamble injected for ALL image-based recipes to force style matching
const IMAGE_FIDELITY_PREAMBLE = `CRITICAL — READ BEFORE DOING ANYTHING:
Study the uploaded image carefully. Your output MUST:
1. MATCH THE ART STYLE of the input. If the input is a cartoon, output a cartoon. If it's a photograph, output a photograph. If it's pixel art, anime, illustration, 3D render, or any other style — match that style exactly.
2. PRESERVE THE SUBJECT. If the input shows a frog, your output must show that same frog. If it shows a cat, keep the cat. If it shows a person, keep the person. Do NOT replace the subject with a different character or a generic person.
3. KEEP THE COMPOSITION. Same framing, same crop, same camera angle.

NEVER convert between art styles unless the recipe explicitly asks for it. A cartoon frog should get rasta elements added in cartoon style. A photo of a dog should remain a photo of that dog.
`;
...
function buildPrompt(recipe, options = {}) {
    ...
    switch (recipe) {
        case 'irie_portrait':
            prompt += `Edit this image. Keep the EXACT same subject — same character, same visual identity, same art style. Do NOT replace the subject with a different character or change the art medium.

TRANSFORMATION INTENSITY: ${INTENSITY_PROMPTS[intensity] || INTENSITY_PROMPTS[3]}

Add these Rastafari elements to the subject (drawn/rendered in the SAME style as the input):
...
2. Thick ropey dreadlocks emerging from under the hat, hanging past the shoulders
3. ${smoking}
4. ${sacred}

BACKGROUND: Add a few cannabis plants with serrated fan leaves behind the subject, slightly out of focus. Warm golden-hour lighting.

STRICT RULES:
- The subject must be IDENTICAL to the input — same character, same features, same style
- Keep the original image's camera angle and crop
- Output style MUST match input style (cartoon→cartoon, photo→photo, illustration→illustration)`;
            break;

        case 'lion_vision':
            prompt += `Edit this image. Transform the subject into an Ethiopian imperial portrait. Keep the EXACT same subject — same character, same visual identity, same art style.

Add these specific elements (rendered in the SAME style as the input):
...
4. Behind the subject: arched stone doorway or pillared colonnade, with warm candlelight and thin tendrils of incense smoke

COLOR PALETTE: Deep crimson, burnished gold, forest green, dark walnut wood tones
LIGHTING: Warm, dim, from the left. Gentle rim light on the crown.
MATCH the art style of the input — if it's a cartoon, make a cartoon. If it's a photo, keep it photorealistic.

The subject must be IDENTICAL to the input image — same character, same features.`;
            break;

        case 'roots_rebel':
            prompt += `Edit this image. Turn the subject into a protest figure / revolutionary portrait. Keep the EXACT same subject — same character, same visual identity, same art style.
...
3. ${smoking}
4. A stern, determined expression (if the subject allows — do NOT distort the features, just adjust intensity slightly)

BACKGROUND: Corrugated zinc metal fence panels, slightly rusted. A painted mural partially visible behind: a clenched fist in red, gold, green.

MATCH the art style of the input — if it's a cartoon, make a cartoon revolutionary. If it's a photo, keep it photorealistic with 35mm film grain.

The subject must be IDENTICAL to the input image.`;
            break;
...
The overall feel should be authentic — like a poster you'd find wheat-pasted on a wall in Kingston, Jamaica in 1978. Not clean or digital-looking.`;
        ...
        case 'chaos_static':
            prompt += `Edit this image. Apply extreme visual distortion and decay while keeping the subject recognizable through the chaos.

DISTORTION LAYER: ${chaosAesthetic}

APPLY THESE EFFECTS HEAVILY:
1. The subject should remain the primary focal point — recognizable but embedded within heavy visual noise
...
DAMAGE OVERLAY: Heavy scratches, chemical stains, and tape residue. The image should look physically damaged — crumpled, burned at the edges, submerged in water, then rescued.
...
The subject must remain recognizable from the input image. MATCH the art style of the input — if it's a cartoon, distort a cartoon. If it's a photo, distort a photo.`;
            break;

        case 'milady_irie':
            prompt += `Edit this image. Transform it with Rastafari elements while STRICTLY maintaining the SAME art style as the input.
