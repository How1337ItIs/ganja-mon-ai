...
| --------------------------------------------- | ------------ | ----------------------- |
| nad.fun, nadapp.net, *.nad.fun, *.symphony.io | Not required | None                    |
...
**IMPORTANT**: The full `api_key` is only shown ONCE when you create it. If you lose it: 1. Use List to find the key's `id` by `key_prefix` or `name` 2. Delete the old key by `id` 3. Create a new key.
